...
class LowResSatellitesService:
    def __init__(self):
        ...
        with open("config/US2020.yaml", "rt") as f:
    ...
    def fetch_imagery_from_sentinel_hub(self, location, satellites='S3'):
        start_date = datetime.datetime.strptime(self.fire_locations.get(location)['modisStartDate'], '%Y-%m-%d')
        end_date = datetime.datetime.strptime(self.fire_locations.get(location)['modisEndDate'], '%Y-%m-%d')
    ...
    def get_request_template(self, location, satellites, start_time, end_time, time, band_name, units, resolution, pixel_scale):
        ...
        roi = self.fire_locations.get(location)['roi']
