...
class DatasetPrepareService:
    def __init__(self, location):
        ...
        self.geometry = ee.Geometry.Rectangle(
            [self.longitude - self.rectangular_size, self.latitude - self.rectangular_size,
             self.longitude + self.rectangular_size, self.latitude + self.rectangular_size])
        self.scale_dict = {"GOES": 2000, "FIRMS": 1000, "Sentinel2": 30, "VIIRS": 500, "MODIS": 500, "Sentinel1_asc": 30, "Sentinel1_dsc":30}
        ...
        for i in range(time_dif.days):
            ...
            for satellite in satellites:
                ...
                if len(max_img.getInfo().get('bands')) != 0:
                    self.download_image_to_gcloud(max_img.toFloat(), satellite, date_of_interest)
    ...
    def batch_downloading_from_gclound(self, satellites):
        for satellite in satellites:
    ...
    def firms_generation_from_csv_to_tiff(self, generate_goes, is_downsample):
        preprocessing = PreprocessingService()
        time_dif = self.end_time - self.start_time
        dataset_pre = DatasetPrepareService(location=self.location)
        for i in range(time_dif.days):
            date_of_interest = str(self.start_time + datetime.timedelta(days=i))
            path = 'data/' + self.location + 'GOES' + '/'
            goes_arr, goes_profile = preprocessing.read_tiff(path + os.listdir(path)[0])
            bbox = [goes_profile.data.get('transform').column_vectors[2][0],
            ...
            for l in range(timestamp_per_day.shape[0]):
                if generate_goes:
                    time_stamp_start, time_stamp_end = self.convert_int_to_timestamp(timestamp_per_day[l], 3)
                    img_collection, img_collection_as_gif = dataset_pre.prepare_daily_image(False, satellite='GOES',
                                                                                            date_of_interest=date_of_interest,
                                                                                            time_stamp_start=time_stamp_start,
                                                                                            time_stamp_end=time_stamp_end)
                    img_to_visualize = img_collection.max()
                    dataset_pre.download_image_to_gcloud(img_to_visualize.toFloat(), 'GOES',
                                                         date_of_interest + '{:04d}'.format(timestamp_per_day[l]))
                fire_data_filter_on_timestamp = np.array(fire_data_filter_on_date_and_bbox[
                                                             fire_data_filter_on_date_and_bbox.acq_time.eq(
                                                                 timestamp_per_day[l])])
                ...
                for k in range(1, fire_data_filter_on_timestamp.shape[0]):
                    ...
                    b2_pixels[cord_y, cord_x] = max(b2_pixels[cord_y, cord_x], record[9])
                ...
                if is_downsample:
                    dst_ds = gdal.GetDriverByName('GTiff').Create(
                        'label/' + self.location + ' label' + '/' + "Cal_fire_" + self.location + satellite + '-' + str(
                            date_of_interest) + '{:04d}'.format(timestamp_per_day[l]) + '_downsampled.tif', image_size[1],
                        image_size[0], 4, gdal.GDT_Float64)
                else:
                    dst_ds = gdal.GetDriverByName('GTiff').Create(
                        'label/' + self.location + ' label' + '/' + "Cal_fire_" + self.location + satellite + '-' + str(
                            date_of_interest) + '{:04d}'.format(timestamp_per_day[l]) + '.tif', image_size[1], image_size[0], 4,
                        gdal.GDT_Float64)
